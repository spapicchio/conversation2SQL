from dotenv import load_dotenv

from conversation2sql.cli_parser import PydanticParser
from conversation2sql.config_input import ConfigReader, ConfigPredictor, ConfigScorer, ConfigPipeline
from conversation2sql.eval import workflow_evaluation_pipeline
from conversation2sql.eval.workflow import EvalPipelineInput

load_dotenv('.env')


def main_launch_eval():
    parser = PydanticParser([ConfigPipeline, ConfigReader, ConfigPredictor, ConfigScorer])
    config_pipeline, config_reader, config_pred, config_scorer = parser.parse_args_and_config()
    data_input = EvalPipelineInput(
        config_reader=config_reader,
        config_predictor=config_pred,
        config_scorer=config_scorer,
        **config_pipeline.model_dump()
    )
    data_output: EvalPipelineInput = workflow_evaluation_pipeline(data_input)
    print(data_output.dataset_with_pred[0])


if __name__ == "__main__":
    main_launch_eval()
