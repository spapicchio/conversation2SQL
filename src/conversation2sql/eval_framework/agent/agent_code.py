...
from langchain.agents.middleware import ToolCallLimitMiddleware, ModelCallLimitMiddleware, ModelRetryMiddleware, \
    ToolRetryMiddleware
...
from conversation2sql.eval_framework.agent.agent_callback import (
    ...
    model_budget_exhausted, TOOL_COSTS
)
...
def _process_agent_response(response: CustomAgentState) -> Any:
    ...
    return {
        **response,
        ...
        'mean_completion_tokens': sum(mean_completion_tokens) / len(mean_completion_tokens),
...
def _extract_ai_metadata(message: AIMessage) -> dict:
    ...
    um = message.usage_metadata or {}  # https://reference.langchain.com/python/langchain-core/messages/ai/UsageMetadata?_gl=1*11ucany*_gcl_au*NDc0Mzc2NTAuMTc3Mjc5MTAyOA..*_ga*MjA2NDMyNTk0Ny4xNzcyNzkxMDI4*_ga_47WX3HKKY2*czE3NzcyODQ1ODckbzQ3JGcwJHQxNzc3Mjg0NTg3JGo2MCRsMCRoMA..

    prompt_tokens = um.get('input_tokens', -1)
    completion_tokens = um.get('output_tokens', -1)
    total_tokens = um.get('total_tokens') or (
        prompt_tokens + completion_tokens if prompt_tokens >= 0 and completion_tokens >= 0 else -1
    )
    ...
    cost_usd = meta.get('_response_cost') or meta.get('response_cost') or meta.get('token_usage', {}).get("cost") or 0.0
    ...
    tool_calls = [
        {
            'tool_name': tc['name'],
            'arguments': tc['args'],
            'tool_cost': TOOL_COSTS.get(tc['name'], -1),  # custom mapping of tool name to cost
        }
