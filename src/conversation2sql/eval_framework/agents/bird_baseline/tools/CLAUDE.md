...
- Result truncation is **row-based, not character-based** (`utils_db_execute.py`): `MAX_RESULT_ROWS = 3` whole rows are shown, then `_more_rows_note(...)` appends `... [showing first 3 of N rows; …]`. Width is left uncapped (the old per-cell 100-char cut in `_format_cell` is gone) so `execute_sql` and `psql_console` output stay comparable. `_execute_query` fetches up to `RESULT_FETCH_LIMIT = 10_000`; a result at the cap is reported as `10000+`. `execute_sql_impl` no longer does any post-format truncation — `_format_result` owns it.
